...
每个 test 内部的每项功能都必须成为可单独调用、可单独开关的模块。保留一键执行，同时支持 PyCharm 右键运行和终端运行，二者调用同一套实现、读取同一套配置。
...
- `model_diagnostics/run_model_diagnostics.py` 旧版通过修改 `sys.argv` 调用引擎，并默认执行多项汇总；首版模块化已改为一次读取、按开关执行。
...
默认指各入口随附的示例配置，不允许入口代码在后台覆盖用户显式配置。存在 `modules` 节时，未列出的模块视为关闭；speed_test 不再读取旧的 `test_*` 测试选择字段。
...
1. 选择已安装所需依赖的 Python/conda 解释器。
2. 修改对应 YAML 的模型、数据路径及 `modules` 开关。
3. 右键运行对应入口；无需填写 Parameters，无需特定 Working directory。
4. 需要临时覆盖时，在 Parameters 使用与终端相同的参数。

现有入口默认配置仍位于原处：测速使用 `speed_test/benchmark_config.yaml`，诊断使用 `model_diagnostics/config/pycharm_run.yaml`，可视化使用 `model_visualization/config/pycharm_run.yaml`。根入口默认使用新增 `config/tools.yaml`。
...
路径：默认配置相对入口文件定位；显式 `--config` 相对终端工作目录解析；新配置的 `project.root` 相对 YAML 所在目录解析，模型/数据/输出相对该 root；命令行显式文件路径相对工作目录。旧配置保留原路径解析规则，通过 `schema_version: 2` 区分并提示迁移，不静默改变路径含义。
...
- 单次运行中，同一配置的标注只读取一次；普通预测只执行一次。测速的 warmup/repeats 属于测量要求，不算重复业务推理。
...
复用 `--run-dir` 时保留旧产物，新增本次 invocation 子目录；清单和 HTML 只引用本次结果，避免关闭模块后误展示旧报告。该行为与旧版覆盖语义的差异需写入迁移说明。

## 11. 实施阶段

1. 建立模块接口、配置解析、执行计划和直接运行/包运行兼容；保留现有入口。
2. 拆分测速、模型信息、显存、导出、构建、一致性和现有可视化功能；落实关闭即不计算。
3. 拆分诊断数据读取与预测接口，消除重复读取和 `sys.argv` 调用；关闭默认常规评估，接入可选现成验证能力。
4. 实现各类诊断独立模块、重叠分析、差图排序和可视化输出。
5. 增加配置迁移说明、PyCharm/终端示例，完成下面的验收。

## 12. 验收标准

- 同一配置在 PyCharm 直接运行、终端脚本运行和 `python -m` 运行时，选择模块和业务结果一致；从其他工作目录启动仍可定位默认配置。
...
- 验证实际使用的旧 YAML 能迁移；保留用户路径；未知开关和冲突配置有可操作的错误提示。
- 有 GPU 时做真实模型测速与一致性冒烟；无 GPU 时记录未验证项，不将替身测试视为真实 GPU 验证。
- 检查导出图片和 HTML 的可读性、相对链接及中文显示；已有结果不会混入新运行。

完成定义：上述模块均能独立选择执行，2/3/4/5 的行为落实，文档中的 PyCharm 和终端用法通过对应验证；仅拆文件但仍全量执行不算完成。
