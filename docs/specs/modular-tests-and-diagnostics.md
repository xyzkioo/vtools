...
默认诊断流程不计算 AP/mAP、阈值扫描、FP 预算和 bootstrap 等汇总；AP/mAP 计算代码已移除。固定工作点的 P/R/F1 仍由一次匹配计算产生，作为差图和错误类型的计数基础，不替代 Ultralytics 的验证结果。阈值扫描仅在用户显式启用、需要 FP/图预算时执行。
...
- 一键开启多个诊断模块时，标注只读一次、预测只生成一次；关闭常规验证时绝不调用 Ultralytics 验证。
