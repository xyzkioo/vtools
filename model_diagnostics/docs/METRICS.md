...
模块化诊断默认只执行逐图错误定位和框关系分析。AP/mAP 等常规验证以 Ultralytics
原生 `val` 输出为准；本页保留 AP 的用途说明用于解读外部结果，本工具不会重复生成
AP 文件。阈值扫描也只有显式开启 `diagnostics.threshold_sweep` 才会执行。

## 指标定义

| 指标 | 计算方式 | 用途 |
|---|---|---|
| AP50、AP75、mAP50–95 | 由 Ultralytics `val` 按其当前指标口径计算 | 观察整体检测质量和定位严格程度 |
...
- 诊断工作点的 P/R/F1 使用一对一匹配；AP/mAP 以 Ultralytics 原生 `val` 为准，最佳候选覆盖率只用于诊断。
